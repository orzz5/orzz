...
class AdminDashboard {
    ...
    markAsCompleted(type, timestamp) {
        console.log(`Marking ${type} with timestamp ${timestamp} as completed`);
    ...
    markAsUncompleted(type, timestamp) {
        console.log(`Marking ${type} with timestamp ${timestamp} as uncompleted`);
        const submission = this.data[type].find(item => item.timestamp === timestamp);
        if (submission) {
    ...
    declineSubmission(type, timestamp) {
        console.log(`Declining ${type} with timestamp ${timestamp}`);
        if (confirm('Are you sure you want to decline/remove this submission?')) {
            ...
            console.log(`Removed ${originalLength - this.data[type].length} items`);
            this.saveLocalData();
            this.renderSubmissions();
